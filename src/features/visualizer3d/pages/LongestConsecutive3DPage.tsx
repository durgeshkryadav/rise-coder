/**
 * LongestConsecutive3DPage.tsx — Full-page Longest Consecutive Sequence 3D visualizer.
 */
...
import {
  ...
  LONGEST_CONSECUTIVE_CODE,
  LONGEST_CONSECUTIVE_LEGEND,
} from '../algorithms/longestConsecutive';
...
const DEFAULT_NUMS = [100, 4, 200, 1, 3, 2];

const LongestConsecutive3DPage: React.FC = () => {
    ...
    <Box sx={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden', bgcolor: '#0a0a1a' }}>
      ...
      <Box sx={topBarSx}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Typography sx={titleSx}>Longest Consecutive — 3D Walkthrough</Typography>
        </Box>
        <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
          <Typography sx={{ fontSize: 13, color: '#aaa' }}>Array:</Typography>
          <TextField size="small" value={numsInput} onChange={(e) => setNumsInput(e.target.value)} placeholder="e.g. 100,4,200,1,3,2" sx={inputSx} />
...
export default LongestConsecutive3DPage;
